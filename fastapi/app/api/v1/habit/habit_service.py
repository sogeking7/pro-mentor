...
def update_user_habit(db: Session, habit_id: int, user_id: int, habit_in: HabitUpdate) -> HabitOut:
    ...
    return habit_repo.update_user_habit(db, user_id=user_id, habit_id=habit_id, habit_in=habit_in)
...
def save_habit_completion(db: Session, habit_id: int, user_id: int, habit_completion_in: HabitCompletionSave) -> \
        Optional[
            HabitCompletionOut]:
    user_habit = habit_repo.get_user_habit(db, habit_id=habit_id, user_id=user_id)
    ...
    if not habit_completion:
        habit_completion = habit_completion_repo.insert_habit_completion(db,
                                                                         habit_id=habit_id,
                                                                         habit_completion_in=habit_completion_in)
