...
def get_habit_completions_query(db: Session, habit_id: int):
    return db.query(HabitCompletion).options(joinedload(HabitCompletion.habit)).filter_by(habit_id=habit_id)
...
def get_habit_completions_range(db: Session, user_id: int, start_date: datetime, end_date: datetime) -> List[
    HabitCompletionOut]:
    habits = (
...
def insert_habit_completion(db: Session, habit_id: int, habit_completion_in: HabitCompletionSave) -> HabitCompletionOut:
...
def update_habit_completion(db: Session, habit_id: int, habit_completion_in: HabitCompletionSave) -> HabitCompletionOut:
